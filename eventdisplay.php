...
include('header.php');
	...
	if(!empty($_POST["search"]["date"])) {			
		...
		$queryCondition .= "WHERE e_date BETWEEN '$fiy-$fim-$fid' AND '" . $date_todate . "'";
	}

	$sql = "SELECT * from event " . $queryCondition . " ORDER BY e_date desc";
...
<table class="table-content ">
	...
	?>
        <tr>
			...
			<td><?php echo $row["e_organiser"]; ?></td>
			<td><?php echo $row["e_location"]; ?></td>
			<td><?php echo $row["e_date"]; ?></td>
			<td><?php echo $row["e_time"]; ?></td>
			<td><?php echo $row["e_discription"]; ?></td>
